...
int CreateHTTPserver()
{
	...
	if (pid == 0) // child process
	{
		...
		else if(!strcmp(strHTTP_requestPath, "/compute"))
		{
			...
			double const limit = 2'000'000.0;
			FuncA funcA;
			std::vector<double> aValues;

			// Mersenne Twister random engine
			std::mt19937 mtre {123};
			std::uniform_real_distribution<double> distr {0.0, limit};

			for (double i = 0.0; i < limit + eps; i++)
			{
				aValues.push_back(funcA.taylor_sine(distr(mtre)));
			}
